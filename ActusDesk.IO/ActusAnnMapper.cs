...
/// <summary>
/// Mapper for converting ACTUS test JSON to AnnContractModel
/// </summary>
public static class ActusAnnMapper
{
    /// <summary>
    /// Load ACTUS ANN test cases from JSON file
    /// </summary>
    ...
    /// <summary>
    /// Map ACTUS terms to AnnContractModel
    /// </summary>
    public static AnnContractModel MapToAnnModel(ActusTerms terms)
    {
        ...
        // Parse principal redemption cycle (ANN-specific)
        if (!string.IsNullOrWhiteSpace(terms.CycleAnchorDateOfPrincipalRedemption))
            model.CycleAnchorDateOfPrincipalRedemption = ParseDate(terms.CycleAnchorDateOfPrincipalRedemption);

        if (!string.IsNullOrWhiteSpace(terms.CycleOfPrincipalRedemption))
            model.CycleOfPrincipalRedemption = NormalizeCycle(terms.CycleOfPrincipalRedemption);
        ...
        if (terms.NextPrincipalRedemptionPayment.HasValue)
            model.NextPrincipalRedemptionPayment = ParseJsonElement(terms.NextPrincipalRedemptionPayment.Value);
        ...
        model.InterestCalculationBase = terms.InterestCalculationBase;
